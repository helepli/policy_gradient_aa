...
class Learner(object):
    ...
    def make_network(self, num_state_var, hidden, lr):
        ...
        def make_function(input, noutput, activation='sigmoid'):
            if self._lstm:
                dense1 = tf.keras.layers.recurrent.LSTM(units=hidden, activation='tanh')(input)
            else:
                dense1 = tf.keras.layers.Dense(units=hidden, activation='tanh')(input)

            dense2 = tf.keras.layers.Dense(units=noutput, activation=activation)(dense1)

            return dense2

        # Neural network with state and option inputs, end two outputs: a distribution
        # over options and an "end" signal
        state = tf.keras.layers.Input(shape=self.make_shape(num_state_var))
        ocurrent = tf.keras.layers.Input(shape=self.make_shape(self._num_options))
        oallowed = tf.keras.layers.Input(shape=(self._total_actions,))              # Mask for options that are allowed to run

        stateoption = tf.keras.layers.concatenate([state, ocurrent])
        pi = make_function(stateoption, self._total_actions, 'linear')          # Option to execute given current state and option
        print(self._total_actions)
        print("oallowed", oallowed)
        probas = tf.keras.layers.Lambda(make_probas, output_shape=(self._total_actions,), arguments={'oallowed': oallowed})(pi)
        critic = make_function(stateoption, 1, 'linear')                        # Expected value of a state-action

        self._model = tf.keras.models.Model(inputs=[state, ocurrent, oallowed], outputs=[probas])
        self._critic = tf.keras.models.Model(inputs=[state, ocurrent], outputs=[critic])

        # Compile model with Policy Gradient loss
        print("Compiling model", end="")
        self._model.compile(optimizer=tf.keras.optimizers.Adam(lr=lr), loss='mse')
        self._critic.compile(optimizer=tf.keras.optimizers.Adam(lr=lr), loss='mse')
        print(" done")
...
def main():
    ...
    parser.add_argument("--advisors", type=str, nargs='*', default=[], help="-actormodel files to use as advisors")
    parser.add_argument("--adviceact", type=float, default=0.0, help="Enable advice if larger than zero. The closer to 1 it is, the stronger the advice is. Values < 1 artificially increase the entropy of advice.")
