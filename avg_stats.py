...
while True:
    ...
    for j, f in enumerate(files):
        while True:
            ...
            if prefix in elements[j] or len(elements[j]) == 0:
                break
    ...
    try:
        ...
        for j in range(len(files)):
            if len(elements[j]) > 0:
                ...
                try:
                    timesteps[j] = int(parts[-3])
                except:
                    timesteps[j] = 0
                try:
                    times[j] = float(parts[-2])
                except:
                    times[j] = 0
        ...
        if i % 1 == 0:
            print(i / 1000., running_mean, running_mean + running_err, running_mean - running_err, sum(timesteps) / N, sum(times) / N)
